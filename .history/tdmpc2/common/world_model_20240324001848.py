...
class WorldModel(nn.Module):
	...
	def __init__(self, cfg):
		...
		self._inv_dynamics = layers.mlp(cfg.latent_dim + cfg.action_dim + cfg.task_dim, 2*[cfg.mlp_dim], cfg.latent_dim, act=layers.SimNorm(cfg))
	...
	def previous(self, z, a, task):
		"""
		Predicts the previous latent state given the current latent state and action.
		"""
		if self.cfg.multitask:
			z = self.task_emb(z, task)
		z = torch.cat([z, a], dim=-1)
		return self._inv_dynamics(z)
