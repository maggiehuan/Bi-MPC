...
class TDMPC2:
	...
	def update_inverse_dynamics(self, obs, action, next_obs, task):
		"""
		Update inverse dynamics model.
		
		Args:
			obs (torch.Tensor): Observation at the current time step.
			action (torch.Tensor): Action at the current time step.
			next_obs (torch.Tensor): Observation at the following time step.
			task (torch.Tensor): Task index (only used for multi-task experiments).
		"""
		...
		loss = F.mse_loss(pred_next_z, next_z)
		loss.backward()
		self.optim.step()
