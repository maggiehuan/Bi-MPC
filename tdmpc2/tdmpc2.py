...
class TDMPC2:
	...
	@torch.no_grad()
	def plan_reversed(self, z, t0=False, eval_mode=False, task=None):
		"""
		Plan a sequence of actions using the learned world model.
		...
		for _ in range(self.cfg.iterations):
			...
			# filtering actions
			# 动作最大值小于threshold
			valid_actions_mask = sampled_actions.abs().max(dim=2)[0] < action_threshold
			valid_actions_mask = valid_actions_mask.unsqueeze(2).repeat(1, 1, self.cfg.action_dim)
	...
	@torch.no_grad()
	def plan_reversed_batch(self, z, t0=False, eval_mode=False, task=None):
		"""
		Plan a sequence of actions using the learned world model.
		...
		for _ in range(self.cfg.iterations):
			...
			for i in range(self.cfg.num_samples):
				final_states = self.forward_rollout(z, actions, task)  
				reconstructed_initial_states = self.inverse_rollout(final_states, actions, task)
				errors = self.calculate_error(z, reconstructed_initial_states)
	...
	@torch.no_grad()
	def plan_reversed_single(self, z, t0=False, eval_mode=False, task=None):
		"""
		Plan a sequence of actions using the learned world model.
	...
	def update(self, buffer):
		...
		self.update_inverse_dynamics(obs, action, task)
